...
- **カーボサイクル運用**（フェーズ2）: トレ日とオフ日でメニューを切り替える
  - トレ日: `python3 scripts/calc.py --menu 筋トレ期_トレ日_2500kcal`
  - オフ日: `python3 scripts/calc.py`（active_menuのデフォルト）
  - その日がトレ日かオフ日かは `config/training_schedule.json` の `scheduled` / `completed` を参照
...
| 1b: マンジャロ減量期 | 完了 | マンジャロ_減量期_2000kcal | -500kcal/day、LBM 72.5〜73.3kgで安定化 |
| 2: 筋トレ期 | **active** | 筋トレ期_オフ日_2000kcal / 筋トレ期_トレ日_2500kcal | カーボサイクル、パーソナルジム週2回 |

## トレーニング予定管理（フェーズ2）

`config/training_schedule.json` で予定と実施履歴を管理する。

- **予定追加**: `scheduled` 配列に `{date, day, status: "planned"}` を追加
- **実施記録**: トレーニング後 `scheduled` から `completed` へ移動
- **キャンセル**: status を `"cancelled"` に変更、ノート記録
- **デフォルトパターン**: 平日(火〜木のいずれか) + 週末(土日いずれか) の週2回
